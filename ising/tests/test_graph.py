...
"""
    ...
    for edg in old_k:
        konig.addedges(
            ising.graph.EdgeFactory.getsingleton().makeedge(
                verts1[edg[0]], verts1[edg[1]], edg[2], directed=False
            )
        )
    kalin.addedges(
        [
            ising.graph.EdgeFactory.getsingleton().makeedge(
                verts2[edg[0]], verts2[edg[1]], edg[2], directed=False
            )
            for edg in new_k
        ]
    )
    ...
    assert konig.getedges()[0].setlength(150) == 150
    assert konig.getedges()[0].getlength() == 150
