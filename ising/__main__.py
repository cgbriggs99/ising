...
Runs the Ising command line program.
    ...
    parser.add_argument("--depth", metavar = "N",
                        default = 10, type = int,
                        help = "The depth of the Metropolis algorithm, if chosen.")
    parser.add_argument("--mc-points", metavar = "N",
                        default = 1000, type = int,
                        help = "Points to check the Metropolis algorithm at.")
    parser.add_argument("--backend", choices = ["monte-carlo", "c", "python"],
                        default = "c", help = "Select which backend to use.")
    ...
    try :
        thermo.PlotValsMethod.getstrat().setthreads(args["threads"])
    except :
        pass
    
    temps = np.linspace(args['low_temp'], args["high_temp"], args["points"])
    ens, endev, magdev = thermo.PlotValsMethod.getsingleton().calc_plot_vals(hamiltonian.PeriodicHamiltonian(
